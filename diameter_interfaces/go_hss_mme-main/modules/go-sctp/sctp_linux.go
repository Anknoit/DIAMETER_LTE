...
// favoriteAddrFamily resolves the appropriate address family (AF_INET or AF_INET6)
// based on the network type and addresses provided
func favoriteAddrFamily(network string, laddr, raddr *SCTPAddr, mode string) (af int, ipv6only bool) {
	...
	if mode == "listen" && (laddr == nil || len(laddr.IPAddrs) == 0) {
		return syscall.AF_INET6, false
	}

	if (laddr == nil || len(laddr.IPAddrs) == 0) && (raddr == nil || len(raddr.IPAddrs) == 0) {
		return syscall.AF_INET6, false
	}
	...
	if laddr != nil {
		for _, addr := range laddr.IPAddrs {
			if addr.IP.To4() != nil {
				ipv4 = true
			} else {
				ipv6 = true
	...
	if raddr != nil {
		for _, addr := range raddr.IPAddrs {
			if addr.IP.To4() != nil {
				ipv4 = true
			} else {
				ipv6 = true
...
// setDefaultSockopts sets the default socket options for an SCTP socket
func setDefaultSockopts(sock int, af int, ipv6only bool) error {
	// Allow reuse of recently-used addresses
	if err := syscall.SetsockoptInt(sock, syscall.SOL_SOCKET, syscall.SO_REUSEADDR, 1); err != nil {
		return err
	}

	// If it's an IPv6 socket, set IPv6 only if requested
	if af == syscall.AF_INET6 {
		if err := syscall.SetsockoptInt(sock, syscall.IPPROTO_IPV6, syscall.IPV6_V6ONLY, boolint(ipv6only)); err != nil {
			return err
		}
	}
...
// boolint converts a boolean to 0 or 1
