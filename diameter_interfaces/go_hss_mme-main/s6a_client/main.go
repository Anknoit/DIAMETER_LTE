...
import (
	...
	"epiphanyHSS/modules/go-diameter/diam/dict"
	"epiphanyHSS/modules/go-diameter/diam/sm"
	"epiphanyHSS/modules/go-diameter/diam/sm/smpeer"
)
...
var (
	...
	host            = flag.String("diam_host", "ola-mme-4", "diameter identity host")
	realm           = flag.String("diam_realm", "ola-mme-4", "diameter identity realm")
...
func sendAIR(c diam.Conn, cfg *sm.Settings) error {
	...
	m.NewAVP(avp.AuthSessionState, avp.Mbit, 0, datatype.Enumerated(0))                              //This AVP indicates the state of the authentication session. A value of 0 typically means that the session is not in progress, and the request is for a new authentication session.
	m.NewAVP(avp.VisitedPLMNID, avp.Vbit|avp.Mbit, uint32(*vendorID), datatype.OctetString(*plmnID)) //This AVP identifies the Public Land Mobile Network (PLMN) that the UE is currently visiting. It includes the vendor ID and the PLMN ID.
...
// run cammand
// go run main.go -addr "127.0.0.1:3868" -network_type "tcp" -diam_host "client" -diam_realm "go-diameter"
